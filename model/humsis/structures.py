...
import numpy as np
import torch
from humsis.data.misc import aggregate_votes_winsorized, assign_faces, box_iou
...
class PersonAndFaceResult:
    def __init__(self, results: Results):
        ...
        self.emotions: List[Optional[int]] = [None for _ in range(n_objects)]
    ...
    def plot(
        ...
        if pred_boxes and show_boxes:
            for bb_ind, (d, age, gender, gender_score, emotion) in enumerate(
                zip(pred_boxes, self.ages, self.genders,
                    self.gender_scores, self.emotions)
            ):
                ...
                if emotions and emotion is not None:
                    emotion_names = ['angry', 'disgust',
                                     'fear', 'happy', 'neutral', 'sad', 'surprise']
                    label += f" { emotion_names[emotion]}"
    ...
    def set_emotion(self, ind: Optional[int], emotion: int):
        if ind is not None:
            self.emotions[ind] = emotion
    ...
    def crop_object(
        ...
        remain_ratio = np.count_nonzero(
            obj_image) / (obj_image.shape[0] * obj_image.shape[1] * obj_image.shape[2])
